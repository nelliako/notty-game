...
from computerLogic.playerDecision import playerDecision
...
def handle_action_steal(game_state: GameState, computer_player_decision: playerDecision, context: TurnContext = None):
    ...
    print(f'Stole a card from Player {game_state.chosen_player.player_id}')
...
def handle_action_swap(game_state: GameState, computer_player_decision: playerDecision = None,
    ...
    if game_state.current_player.type == PlayerType.HUMAN:
        ...
        if user_input == len(game_state.current_player.hand):
            context.has_swapped_card = True
            game_state.deck.add_cards([card_drawn])
            game_state.deck.shuffle_deck()
            return

        game_state.current_player.take_card(card_drawn)
        discarded_card = game_state.current_player.lose_card(user_input)
    ...
    # Choose the card to discard when player is computer_player_decision
        ...
        # ANAN Function
        card_index = computer_player_decision.discard_card_from_hand()

        discarded_card = None

        # If we choose the drawn_card, the hand does not change
        if card_index != len(game_state.current_player.hand):
            discarded_card = game_state.current_player.lose_card(card_index)
            game_state.current_player.take_card(card_drawn)
