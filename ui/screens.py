        ...
        while not self.close:
            for event in pygame.event.get():
                ...
                try:
                    ...
                    print(f"[Error] process_event failed: {e}")
                    # Continue loop to avoid crashing; could add traceback if needed
            ...
            try:
                ...
                print(f"[Error] draw/update failed: {e}")
                # Keep running to diagnose without hard crash
...
class playScreen(screenBase):
    ...
    def draw_guide(self):
        ...
        text_x = panel_x + 90  # õ╗Ä70µö╣µêÉ90´╝îÕåìÕó×ÕèáÕÀªÞ¥╣ÞÀØ
        text_y = panel_y + 30 + self.guide_scroll

        start_y = panel_y + 30
        max_draw_y = panel_y + target_height - 30

        max_text_width = target_width - 180  # õ╗Ä140µö╣µêÉ180´╝îÕÀªÕÅ│ÕÉä90pxÞ¥╣ÞÀØ´╝îµëÇõ╗ÑÕçÅ180
    ...
    def process_event(self, event):
        ...
        if len(self.game_state.current_player.hand) > 20:
            # Note: avoid hard crash; log warning instead
            print(f"[Warning] {self.game_state.current_player.name}'s hand exceeds 20 cards! Continuing.")
        ...
        for i, player in enumerate(all_players):
            if len(player.hand) > 0:
                ...
                for card_index, card in enumerate(player.hand):
                    ...
                    if not self.is_stealing and not self.is_trading and card_vis.contains_point(mx, my):
                        card_vis.hovered = True
